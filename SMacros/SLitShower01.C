{
  char *name    = "SLitShower01";
  char *listing = "SLitShower01";
  ...
  //
  const char *comTL = "Cascade is an e+ of 100 Gev";
  ...
  */
  ...
  //____________________________________________________________________________
  //
  TGeoManager *geom = new TGeoManager("setup","SLitShower01 of new Litrani");
  //____________________________________________________________________________
  //
  // Materials and media
  //____________________________________________________________________________
  ...
  const Double_t stepy  = 1.0;       //Cascade is displaced along Oy in steps stepy
  const Double_t starty = -2.0;      //Start position of cascade in y
  const Double_t endy   =  2.0;      //End   position of cascade in y
  ...
  TString path = "/TOP_1";
  TVector3 StartCasc(0.0,posy,startz);
  TVector3 AxisCasc(0.0,0.0,1.0);
  TLitCascade *casc;
  if (inlocal)
    casc = new TLitCascade("e+","e+",path.Data(),"PbWO4",-11,100.0,StartCasc,AxisCasc);
  else
    casc = new TLitCascade("e+","e+","PbWO4",StartCasc,AxisCasc,-11,100.0);
