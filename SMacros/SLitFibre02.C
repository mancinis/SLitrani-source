{
  char *name    = "SLitFibre02";
  char *listing = "SLitFibre02";
  char *upcom   = "Setup with fibre, crystal and 2 APDs";
  ...
  // are 2 APDs without thin slice having the form of smaller TGeoBBox. On the bottom of the
  // crystal is a fibre of shape TGeoTube. Between the APDs and the crystal, there is a slice 
  ...
  // sum of the crystal, the 2 APDs and the fibre. The use of union for defining the aluminium wrapping
  // is necessary in order that no overlap occurs and that all 6 faces of the crystal be covered
  // with aluminium, including the part of the upper face not covered by the APDs, and including the
  ...
  // a TGeoBBox made of "totabsorbing", slightly bigger than the APDs and containing them and included
  ...
  // extrusion.
  //  As explained for instance in the class description of TLitVolume, a fundamental characteristics
  // of the TGeo geometry is that a TGeoVolume defined once may be inserted many times in the setup,
  // at different places and with different orientations. This is the case for the APDs and their
  // wrapping in this example. APD with wrapping is defined only once, but is inserted twice by
  // 2 calls to AddNode(). With the old Litrani, we would have to define 2 APDs.
  //  Everything is contained in the TGeoBBox "TOP", defining the WCS, world coordinate system.
  //
  ...
  */
  ...
  //____________________________________________________________________________
  //
  TGeoManager *geom = new TGeoManager("setup","SLitFibre02 of new Litrani");
  //____________________________________________________________________________
  //
  // Materials and media
  //____________________________________________________________________________
  ...
  //
  const Double_t crys_dx   = 3.0;
  const Double_t crys_dy   = 3.0;
  ...
  const Double_t sep_dy = 0.2; //half the separation between the 2 edges of the 2 APDs
  //
  ...
  //
  // Translation of APDs in Oy
  //
  Double_t ty = sep_dy + wid[ns] + apd_dy;
  //
  ...
  //
  // Dimensions of TGEOBBox of alu around APDs and glue of APDs [partially] "APDWS"
  //
  Double_t alua_dx = apd_dx + wid[ns];
  Double_t alua_dy = ty + apd_dy + wid[ns];
  Double_t alua_dz = apd_dz + glueapd_dz[ns];
  Double_t edge    = crys_dy - alua_dy;
  gCleanOut->MMR(info,"SLitFibre02","Free space from edge of crystal","edge",edge,"CINT");
  if (edge<0.0) gCleanOut->MM(error,"SLitFibre02","Overriding crystal","CINT");
  //
  ...
  // crystal [or equivalently, with respect to the composite shape "rev_shape"] along Oz.
  //  t1 is used for the creation of composite shape, so it has to be named and registered
  // by a call to RegisterYourself().
  //
  ...
  t2->RegisterYourself();
  //  t3a and t3b are for positionning the 2 TGeoBBox of "totabsorbing" around APD and around
  // glue of APD with respect to the TGeoBBox of aluminium around crystal [or equivalently,
  // with respect to the composite shape "rev_shape"].
  //
  const Double_t t_tot_crys_z = crys_dz + alua_dz + ws2;
  TGeoTranslation *t3a = new TGeoTranslation("t3a",0.0,-ty,t_tot_crys_z);
  TGeoTranslation *t3b = new TGeoTranslation("t3b",0.0, ty,t_tot_crys_z);
  ...
  rev->AddNode(tot,1,t3a);
  rev->AddNode(tot,2,t3b);
  //
  ...
  lit_fib->SetEmission(sinuscosinus,30.0,emisdir,"",kFALSE,source,kTRUE,facedir);
  //
  geom->CloseGeometry();
