// @(#)root/html:$Id: TLitWave.cpp 2008-06-11
...
//         -------------------------------------------------------------
//
//  TLitWave contains all information about one wave in the ICS ( coordinate system
...
//      1 : it will have to be simply added to fBeta, the irrelevant absolute phase
//          of the wave
...
//
TLitWave::TLitWave(const char *name,const char *title): TNamed(name,title) {
  //Constructor
  fEva = kFALSE;
}
void TLitWave::Split(Bool_t isanisotropic) {
...
Bool_t TLitWave::VerifySplit() const {
  ...
  if (a>small) {
    bad = kTRUE;
    gCleanOut->MMR(error,"VerifySplit","Bad decomposition of the Electric field","a",a,ClassName());
  }
  return bad;
}
Bool_t TLitWave::VerifyStart(Double_t n, const TMat33 &m) const {
  //Verify starting conditions of incident wave in case of anisotropic medium
  const Double_t small = 2.5e-4;
  Bool_t bad = kFALSE;
  Double_t a,b,c,bb;
  TVector3 v;
  TMat33 aam;
  ...
  if (bb>small) {
    bad = kTRUE;
    gCleanOut->MMR(error,"VerifyStart","Bad initial condition","bb",bb,ClassName());
  }
  return bad;
}
