// @(#)root/html:$Id: TVisuSLit.cpp 2008-06-11
...
/* BEGIN_HTML
<div align="center">
<h2>Visualize the ROOT files produced by SLitrani.</h2>
</div>
<p>After having launched <b><code>VisuLitrani</code></b>, please open a <b><code>SLitrani</code></b> 
...
contained in the <b><code>SLitrani</code></b> file. The first item of the <b><code><a href="http://root.cern.ch/root/html/TGListBox.html">TGListBox</a></code></b> 
...
used for the generation of the photons of this <b><code>SLitrani</code></b> <b><code>.root</code></b> 
file.
...
for the generation of the photons of this <b><code>SLitrani</code></b> <b><code>.root</code></b> 
file.
</ol>
...
<p>Notice that the canvas used by <b><code>TVisuSLit</code></b> [ the canvas contained 
...
You can also call all the methods of <b><code>TVisuSLit</code></b> from the command 
prompt window, using the global pointer <b><code>gVisuSLit</code></b>.</p>
...
<p>The global size of the window used by <b><code>TVisuSLit</code></b> is set in 
...
You can adjust the size of the window of <b><code>TVisuSLit</code></b> changing this 
value of 200, but you have the to recompile and relink <b><code>TVisuSLit</code></b>.</p>
<p></p>
END_HTML */
//
TVisuSLit::TVisuSLit(const TGWindow *p,UInt_t w, UInt_t h):
TGMainFrame(p,w,h) {
  ...
  fWidthGen  = w;
  fHeigthGen = h;
  fTextBR          = "Please File/Open a SLitrani file";
  PrepareMenus();
...
//  fEcanvas = new TRootEmbeddedCanvas("Ecanvas",fHFrame1,w-180,h-100);
  ...
  fRadioPar->Connect("Pressed()","TVisuSLit",this,"DoRadio()");
  fButtonGroup->AddFrame(fRadioRun,fL8);
  fRadioRun->Connect("Pressed()","TVisuSLit",this,"DoRadio()");
  fButtonGroup->AddFrame(fRadioDet,fL8);
  fRadioDet->Connect("Pressed()","TVisuSLit",this,"DoRadio()");
  fButtonGroup->AddFrame(fRadioFit,fL8);
  fRadioFit->Connect("Pressed()","TVisuSLit",this,"DoRadio()");
  fButtonGroup->AddFrame(fRadioSpec,fL8);
  fRadioSpec->Connect("Pressed()","TVisuSLit",this,"DoRadio()");
  ...
  fOneTwoButton->Connect("Clicked()","TVisuSLit",this,"ToggleState()");
  ...
  fLinLogButton->Connect("Clicked()","TVisuSLit",this,"ChangePadLogY()");
  ...
  fColorButton->Connect("Clicked()","TVisuSLit",this,"ToggleColor()");
  ...
  fPrintButton->Connect("Clicked()","TVisuSLit",this,"PrintCanvas()");
...
void TVisuSLit::ChangePadLogX() {
  // Go from lin to log or vice-versa in x
  gTwoPad->ChangePadLogX();
}
void TVisuSLit::ChangePadLogY() {
  // Go from lin to log or vice-versa in y
  gTwoPad->ChangePadLogY();
}
void TVisuSLit::CloseFile(Bool_t AlsoEntries) {
  //To close the SLitrani .root file
  if (!fFileClosed) {
    Int_t nlb;
    fLitFile->Close();
    delete fLitFile;
...
void TVisuSLit::DoStat() {
  ...
  if (!fFileClosed) {
    ...
    p = SelectRun();
    if (fListBoxItem == 1) gLitGs->DoStat();
    else                   gLitCs->DoStat();
    gTwoPad->ChangePad();
  }
  else cout << "TVisuSLit::DoStat ERROR : Please open a SLitrani file" << endl;
}
void TVisuSLit::DrawNextFit() {
...
void TVisuSLit::DrawNextSpectrum() {
  // Next fit in fFitColl is drawn
  Bool_t nospec = kTRUE;
  if (gTwoPad) {
    if (fSpecColl) {
...
void TVisuSLit::HandleMenu(Int_t id) {
...
void TVisuSLit::InitRef() {
  // Set references of program
  TAuthor *a;
  fRef = new TReference("VisuLitrani","Visualization of ROOT files produced by SLitrani","3_7",
...
void TVisuSLit::OpenFileR() {
  //Open SLitrani file for read
  fCanvas->cd();
...
void TVisuSLit::PrepareMenus() {
  ...
  fMenuFile->Connect("Activated(Int_t)", "TVisuSLit", this,"HandleMenu(Int_t)");
  fMenuBar = new TGMenuBar(this, 1, 1, kHorizontalFrame);
  fMenuBar->AddPopup("&File", fMenuFile, fLMenuItem);
  AddFrame(fMenuBar, fLMenu);
}
void TVisuSLit::PrintCanvas() {
  // Save canvas as a file. Default jpg
  gTwoPad->SaveCanvas();
}
TLitResults *TVisuSLit::SelectRun() {
  ...
  if (!fFileClosed) {
    ...
    if (selected==1) p = gLitGs;
    else {
      if ((!gLitCs) || (fListBoxItem != selected)) {
        k = selected - 2;
        fLitNb += fLitTree->GetEntry(k,1);
        krun = gLitCs->GetRun();
        s    = gLitCs->GetName();
      }//end if ((!gLitCs) || (fListBoxItem != selected))
      p = gLitCs;
    }//end else if (selected==1)
    fListBoxItem = selected;
  }//end if (!fFileClosed)
  else cout << "TVisuSLit::SelectRun ERROR : file has been closed" << endl;
  return p;
}
void TVisuSLit::ToggleColor() {
